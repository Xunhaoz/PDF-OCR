...
def rotate_correction_visualize():
    img = cv2.imread('output_images/FPFI07.png')
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # cv2.imshow('gray', cv2.resize(gray, (1000, 1500)))

    ret, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
    # cv2.imshow('thresh', cv2.resize(thresh, (1000, 1500)))
...
def rotate_correction_batch(source='output_images', dest='rotated_images'):
    filenames = []
...
def clip_batch(source='rotated_images', dest='clipped_images'):
    filenames = []
    for path, names, filenames in os.walk(source):
        filenames = list(filter(lambda x: ".png" in x, filenames))
...
def voted_extract(img: PIL.Image.Image) -> str:
    enhancement_factors = [i / 10 for i in range(15, 26, 2)]
    radiuses = [i / 10 for i in range(15, 26, 2)]
    vote_dict = {}
...
def extract_png_1(img_path="", config_path="clipped_grid_1.json"):
    ...
    for x_i, x in enumerate(X[:-1]):
        for y_i, y in enumerate(Y[:-1]):
            cropped_img = enhance_img.crop((x, y, X[x_i + 1], Y[y_i + 1]))
            content = pytesseract.image_to_string(cropped_img, lang='eng', config=r'--psm 6 outputbase digits')
            result.append(content.replace("\n", ""))

    cropped_img = enhance_img.crop((date_pt[0][0], date_pt[0][1], date_pt[1][0], date_pt[1][1]))
    date_time = pytesseract.image_to_string(cropped_img, lang='eng', config=r'--psm 6 outputbase digits')
...
def extract_png_2(img_path="", config_path="clipped_grid_2.json"):
    with open(config_path, 'r') as file:
        data = json.load(file)
    ...
    cropped_img = enhance_img.crop((ic_forced[0][0], ic_forced[0][1], ic_forced[1][0], ic_forced[1][1]))
    ic_forced = pytesseract.image_to_string(cropped_img, lang='eng', config=r'--psm 6 outputbase digits')
    ic_forced = ic_forced.replace("\n", "")

    result = [[result[i * 8 + j] for j in range(8)] for i in range(6)]
    result[0].append(ic_forced)
...
def extract_png_3(img_path="", config_path="clipped_grid_3.json"):
    with open(config_path, 'r') as file:
        data = json.load(file)
    ...
    for x_i, x in enumerate(X[:-1]):
        for y_i, y in enumerate(Y[:-1]):
            ...
            content = voted_extract(cropped_img)
            # content = pytesseract.image_to_string(cropped_img, lang='eng', config=r'--psm 6 outputbase digits')
            result.append(content.replace("\n", ""))
...
# extract_png_1("clipped_images/demo.png")
# extract_png_1("clipped_images/FPFI07.png")
# extract_png_1("clipped_images/FPFI06.png")
# extract_png_1("clipped_images/FPFI05.png")
# extract_png_1("clipped_images/FPFI04.png")
# extract_png_2("clipped_images/FPFI02.png")
extract_png_3("clipped_images/FPFI03.png")
